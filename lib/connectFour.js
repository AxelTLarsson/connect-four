...
*/
export {
    setup, makeMove, nextTurn, isWon, currentPlayer, currentPlayerId, getInstance, restoreInstance, updateHighScore, getHighscore, restoreHighscore
}
...
function makeMove(col) {
    ...
    if (row === false) {
        audit.log(`${player} tried to drop a token in the column ${col}`);
        return false;
    } else {
        instance.board[row][col] = instance.turn;
        audit.log(`${player} dropped a token in column ${col}`);
        return row;
    }
}
...
function isWon(row, col) {
    ...
    if ((rightPos.j - leftPos.j) > 4) {
        ...
        audit.log(`${instance.players[player]} wins the game with a horizontal connection!`);
        return true;
    }
    ...
    if ((upPos.i - downPos.i) > 4) {
        audit.log(`${player} wins the game with a vertical connection!`);
        return true;
    }
    ...
    if ((upLeftPos.i - downRightPos.i) > 4) {
        audit.log(`${player} wins the game with a diagonal connection!`);
        return true;
    }
...
function setup(playerNames, xSize, ySize) {
    audit.log(`Setting up a new game of size ${xSize} by ${ySize}`);
